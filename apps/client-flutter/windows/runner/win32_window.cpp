...
constexpr double kMinAspectRatio = 0.62;
constexpr double kMaxAspectRatio = 2.2;
...
SIZE GetMinimumWindowSize(HWND hwnd) {
...
void ApplyResizeLimits(HWND hwnd, UINT edge, RECT* rect) {
  if (rect == nullptr) {
    return;
  }

  const SIZE minimum = GetMinimumWindowSize(hwnd);
  LONG width = std::max(rect->right - rect->left, minimum.cx);
  LONG height = std::max(rect->bottom - rect->top, minimum.cy);

  const double ratio = static_cast<double>(width) / static_cast<double>(height);
  if (ratio < kMinAspectRatio) {
    width = static_cast<LONG>(std::ceil(height * kMinAspectRatio));
  } else if (ratio > kMaxAspectRatio) {
    height = static_cast<LONG>(std::ceil(width / kMaxAspectRatio));
  }

  width = std::max(width, minimum.cx);
  height = std::max(height, minimum.cy);
...
Win32Window::MessageHandler(HWND hwnd,
  ...
  switch (message) {
    ...
    case WM_SIZING: {
      ApplyResizeLimits(hwnd, static_cast<UINT>(wparam),
