...
fn normalize_client_metadata_status(status: &str) -> String {
    match status.trim() {
...
async fn mark_client_replica_ready(pool: &DbPool, file_id: i64, now: &str) -> Result<()> {
    sqlx::query(
        r#"
        UPDATE media_replicas
        SET device_id = (
...
async fn attach_client_file_to_track(
    pool: &DbPool,
    file_id: i64,
    target_track_id: i64,
) -> Result<i64> {
