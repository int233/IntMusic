...
async fn list_client_library_roots(
    State(state): State<AppState>,
...
async fn upsert_client_library_manifest(
    State(state): State<AppState>,
...
async fn remove_client_library_root(
    State(state): State<AppState>,
...
async fn apply_client_mutations(
    State(state): State<AppState>,
...
#[derive(Debug, Deserialize)]
struct DistributionDeviceQuery {
    device_id: String,
}

async fn next_distribution_task(
    State(state): State<AppState>,
...
async fn next_distribution_source_task(
    State(state): State<AppState>,
    Query(query): Query<DistributionDeviceQuery>,
...
async fn upload_distribution_source(
    State(state): State<AppState>,
...
async fn report_distribution_source_task(
    State(state): State<AppState>,
    Path(task_id): Path<String>,
...
async fn receive_distribution_source(
    body: Body,
    headers: &HeaderMap,
    temporary: &FsPath,
    expected_size: i64,
...
async fn report_distribution_task(
    State(state): State<AppState>,
    Path(task_id): Path<String>,
    Json(payload): Json<DistributionTaskProgress>,
) -> ApiResult<protocol::DistributionJobSummary> {
...
async fn distribution_task_content(
    State(state): State<AppState>,
    Path(task_id): Path<String>,
    Query(query): Query<DistributionDeviceQuery>,
    headers: HeaderMap,
...
fn start_distribution_transcode_worker(state: AppState) {
...
async fn prune_transcode_cache(state: &AppState) {
...
async fn cleanup_distribution_relay_sources(state: &AppState, job_id: &str) {
...
async fn remove_distribution_relay_path(state: &AppState, path: &FsPath) {
...
async fn track_recording_candidates(
    State(state): State<AppState>,
    Path(track_id): Path<i64>,
...
async fn detach_track_recording(
    State(state): State<AppState>,
    Path(track_id): Path<i64>,
) -> ApiResult<protocol::TrackMediaProfile> {
